...
def read_plugins_list():
    ...
    def item_to_path(item):
        if os.path.isdir("./plugins/%s" % item):
            js_file = next(file_name for file_name in os.listdir("./plugins/%s" % item) if file_name.endswith(".js"))
...
def save_graph(graphs: list = Body(...)):
    ...
    for graph in graphs:
        if not (graph["name"]) in os.listdir("./graphs"):
            ...
            count += 1
    if count == len(graphs):
        return {"status": "SUCCESS", "code": 200, "count": count}
...
def update_graph(graphs: list = Body(...)):
    ...
    for graph in graphs:
        if file_list[graph["id"]]:
            if "content" in graph:
                with open(os.path.join("./graphs/", file_list[graph["id"]]), "w") as file:
                    file.write(graph["content"])
    ...
    for conf in workspaces:
        ...
        if file_name in file_list:
            if 'content' not in conf:  # if no content change title
                ...
                with open(os.path.join("./workspaces", file_name), "w") as file:
                    file.write(json.dumps(configuration))
            else:
                with open(os.path.join("./workspaces", file_name), "w") as file:
