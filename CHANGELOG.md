...
- param `octetCount` to `getPlugin()` method to get a version higher than requested

### Changed

- now `getInstance()` method does not throw an error if the given GUID does not exist
- HeaderPanel plugin version in pages
- ProfilePanel plugin version in pages
